...
export class GameEngine {
  ...
  private async processEvent(event: Event): Promise<void> {
    ...
    } else if (event.type === "BIRD_PLAYED") {
      // First trigger the white power (WHEN_PLAYED) for the just-played bird
      await this.processWhitePower(event as BirdPlayedEvent);
      // Then trigger pink powers for other players
  ...
  /**
   * Trigger the white power (WHEN_PLAYED) for a just-played bird.
   * Only executes if the bird has a power with trigger: "WHEN_PLAYED".
   * White powers can trigger recursive bird playing (e.g., playAdditionalBirdInHabitat),
   * which will emit additional BIRD_PLAYED events handled by processEvent().
   */
  private async processWhitePower(event: BirdPlayedEvent): Promise<void> {
    const bird = this.gameState.findBirdInstance(event.birdInstanceId);
    if (!bird) return;

    const power = bird.card.power;
    if (!power || power.trigger !== "WHEN_PLAYED") return;

    const execCtx = this.createActionExecutionContext();
    const result = await this.actionProcessor.executeSinglePower(
      event.birdInstanceId,
      event.playerId,
      execCtx
    );

    // Process any events generated by the power (enables recursive bird playing)
    for (const evt of result.events) {
      await this.processEvent(evt);
    }
  }
  ...
  applyGainFood(effect: Effect & { type: "GAIN_FOOD" }): void {
    ...
    // Add food to player's supply (skip WILD - it's not a real food type)
    for (const [foodType, count] of Object.entries(effect.food)) {
      if (count && count > 0 && foodType !== "WILD") {
  ...
  applyDrawCards(effect: Effect & { type: "DRAW_CARDS" }): void {
    ...
    // Draw from deck - handle empty deck gracefully
    if (effect.fromDeck > 0) {
      try {
        ...
        if (error instanceof Error && error.message === "All cards are in use") {
          // Deck and discard are empty - this is a valid game state, not an error
          // Player simply doesn't draw any cards from the deck
        } else {
  ...
  private buildEligibleActionsAndRewards(
    ...
    // LAY_EGGS: Eligible if player can lay eggs OR has brown powers in Grassland to activate
    // Even if all birds are at full egg capacity, the player may want to activate Grassland
    // just to trigger brown powers on their Grassland birds.
    const remainingCapacities = player.board.getRemainingEggCapacities();
    const hasRemainingEggCapacity = Object.keys(remainingCapacities).length > 0;
    const hasBrownPowersInGrassland = player.board.getBirdsWithBrownPowers("GRASSLAND").length > 0;
    const canLayEggs = hasRemainingEggCapacity || hasBrownPowersInGrassland;
    if (canLayEggs) {
